...
int main() {
    ...
    format_stride_length(text, sizeof(text), 750, ActivityUnits::imperial);
    assert(std::strcmp(text, "STRIDE  29.5 in") == 0);
    format_stride_length(text, sizeof(text), 750, ActivityUnits::metric);
    assert(std::strcmp(text, "STRIDE  0.75 m") == 0);
    assert(next_stride_length(750, ActivityUnits::metric) == 800);
    assert(next_stride_length(762, ActivityUnits::imperial) == 787);
}
